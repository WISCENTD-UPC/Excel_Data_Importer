		...
		if( !hasErrors )
		{
			...
			if( !hasErrors )
			{
				...
				//		if (sheet.period_type == YEARLY_PERIOD) {
							lastColumn = distanceBetween(sheet.period_dim_1, lastColumnForRow(resultArray[sheet_no-1], sheet.period_dim_2));
					//	} else {
					//		lastColumn = sheet.period_length;
					//	}
						var lastRow = lastRowForColumn(resultArray[sheet_no-1], sheet.orgUnit_dim);
						var nbOfPeriods = (lastColumn + de_per_period )  / de_per_period;
						...
						for (var i = 0; i<nbOfPeriods; i++){
							...
							for (var row = sheet.data_starting_row; row <= lastRow; row++){
								orgUnit = getCellDataRC(sheet_no, sheet.orgUnit_dim, row);
								if (orgUnit.length == 11) {
									for (var de_idx = 0; de_idx < de_per_period; de_idx++){
										var de = sheet.data_des[de_idx];
										dataValue = {};									
										dataValue.period = period;
										dataValue.dataElement = de.de_code;
										dataValue.categoryOptionCombo = de.cocuid;
										dataValue.orgUnit = orgUnit;
										var testDim = nextDim(dim1, de_idx);
										dataValue.value = getCellDataRC(sheet_no, nextDim(dim1, de_idx), row);
										console.log(dataValue);
										dataValues.push(dataValue);
									}
								}
