...
function sheet_to_formulae(sheet, sheetNum, sheetLastColumn) {
	var cmds, y = "", x, val="";
	if(sheet == null || sheet["!ref"] == null) return "";
	...
	//sheetEndRows[sheetNum] = encode_row(r.e.r);
	//console.log("End col : " + sheetNum + " : " + sheetLastColumn )
	...
	for(var R = r.s.r; R <= r.e.r; ++R) 
	{
		...
		for(C = r.s.c; C <= r.e.c; ++C) 
		{	
			if( (cols[C].length < sheetLastColumn.length) || (cols[C].toLowerCase() <= sheetLastColumn.toLowerCase() && cols[C].length <= sheetLastColumn.length ) )
			{
				y = cols[C] + rr;
				x = sheet[y];
				...
				{
					cmds[y] = val;
					sheetEndRows[sheetNum] = rr;
				}
					
				
				//console.log( "Printing each row: "+ val + "  ** " + sheetEndRows[sheetNum] );
				++i;
